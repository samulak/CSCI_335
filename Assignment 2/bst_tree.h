...
/**
    ...
    */
    float average_depth_of_tree(BinaryNode* t, float depth){
    	if (t == nullptr)
    	   return 0;
    	else
    ...
    */
     ...
     * Set the new root of the subtree.
     */
    void remove( const Comparable & x, BinaryNode * & t, int &cnt )
    {
        if( t == nullptr )
     ...
     */
    BinaryNode * findMin( BinaryNode *t, int &cnt ) const
    {
        if( t == nullptr )
            return nullptr;
        if( t->left == nullptr )
     ...
     * t is the node that roots the subtree.
     */
    bool contains( const Comparable & x, BinaryNode *t, int &cnt  ) const
    {
        if( t == nullptr )
            return false;
        else if( x < t->element )
